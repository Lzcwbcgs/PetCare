...
type AiSession struct {
	...
	ModelType      string    `json:"modelType"      orm:"model_type"       description:"模型类型，如api/local"`    // 模型类型，如api/local
	...
	RagEnabled     int       `json:"ragEnabled"     orm:"rag_enabled"      description:"是否启用RAG：1是，0否"`      // 是否启用RAG：1是，0否
	RetrievalCount int       `json:"retrievalCount" orm:"retrieval_count"  description:"最近一次召回片段数"`         // 最近一次召回片段数
	...
	LastMessageAt  time.Time `json:"lastMessageAt"  orm:"last_message_at"  description:"最后消息时间"`            // 最后消息时间
